...
**Shopee, mỗi xung 3 call:**

| # | Call | Mục đích |
|---|---|---|
| 1 | `get_product_level_campaign_id_list` (1 trang) | bắt **campaign mới tạo** trong ngày (chính là ca cắn tiền hay gặp). Id lạ → gọi thêm `setting_info` chỉ cho id mới (hiếm) |
| 2 | `get_product_campaign_daily_performance` với `start=end=hôm nay` | cập nhật dòng `today` của ≤100 campaign — đúng dòng rule spike đọc |
| 3 | `get_total_balance` | số dư ví → **ghi DB** `Channel.adsWalletBalance` + `adsWalletSyncedAt` |
...
trang 1 `searchCampaignList` (bắt campaign mới + cờ hết tiền ví).
...
### Tầng B — LƯỢT ĐẦY ĐỦ, mỗi 6 giờ (giữ như hôm nay)

Cấu hình campaign, cửa sổ 7 ngày (Lazada attribution nở dần), backfill 30 ngày khi
nối / lần đầu. Nút Làm mới và nudge >30' khi mở trang vẫn kích tầng này.
...
|---|---|---|---|
| Shopee | 3 × 48 = 144 | 4 × 4 = 16 | **160** |
| Lazada | 2 × 48 = 96 | 58 × 4 = 232 | **328** |

| G | Shopee call/ngày | QPS trung bình | Lazada call/ngày |
|---|---|---|---|
| 300 | 48.000 | 0,6 | 98.000 |
| 3.000 | 480.000 | 5,6 | 984.000 |
| 10.000 | 1.600.000 | 18,5 | 3.280.000 |

Kết luận số:

- **Shopee**: 5,6 QPS ở 3.000 gian là mức phải hỏi Shopee. Token bucket 3/s mặc định
...
3. Tầng A xung: cột `nextAdsPulseAt`, `adsWalletBalance`, `adsWalletSyncedAt` trên
