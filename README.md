...
| POST | `/api/webhooks/tiktok` | **[TikTok thật]** Webhook đơn hàng (type 1/2/3/4/11/12) + thu hồi/hết hạn ủy quyền (6/7): verify chữ ký → ghi hàng đợi bền `tiktok_webhook_logs` → ack; worker lấy chi tiết đơn → upsert + trừ/hoàn kho (idempotent, retry 3) | 🔑 Chữ ký HMAC |
...
2. **Nhận** sự kiện đơn hàng type 1 (đổi trạng thái) / 2 (hoàn lại bản cũ) / 3 (đổi địa chỉ) / 4 (kiện hàng) / 11 (hủy) / 12 (hoàn trả) và type 6/7 (thu hồi / hết hạn ủy quyền) — số type đối chiếu bảng Quản lý webhook trên Console 16/09; loại khác ack **200** để TikTok khỏi gửi lại.
...
**Ngang hàng Shopee/Lazada (rà soát toàn cây code 16/09/2026, shop nhà and.not.or đã ủy quyền):**

- **Vòng quét tự động** (`workers/order-auto-sync.ts`): tầng nhanh quét đơn theo `update_time` 2 ngày + đơn hoàn (`tiktok/returns-sync.ts`, Return & Refund API: giải pháp hoàn, tiền hoàn, SKU trả, tracking chiều hoàn — không bịa giá) + cứu đơn giao thất bại (`tiktok/delivery-fail.ts`, tracking API, cùng hàng đợi vé `DeliveryTrackingTask` với Shopee; auto-chat ghi SKIPPED vì chưa nối chat); tầng giờ đối soát bản kê 7 ngày + đợt chi tiền (`tiktok/payouts.ts` → `WalletWithdrawal`, ví sàn = đợt đang PROCESSING). Không có tầng ads.
- **Sản phẩm & tồn kho**: `marketplace/adapters/tiktok-adapter.ts` (products/search, mỗi SKU một dòng, `externalId` = `productId-skuId`, tồn Σ kho, kho giữ tồn → `channelStockLocationId`); `PUSHABLE_CHANNELS` có TikTok → worker đẩy tồn gọi `inventory/update` theo kho (thiếu kho thì tra `logistics/warehouses` một lần), đối soát 6h, so khớp ở Hàng hóa.
- **Chuẩn bị hàng & in vận đơn**: `services/fulfillment/tiktok.ts` (handover_time_slots → ship package PICKUP/DROP_OFF → tracking → shipping_documents A6 PDF, xin bản gộp `SHIPPING_LABEL_AND_PACKING_SLIP` có danh sách sản phẩm như Seller Center, sàn từ chối mới lùi về tem trần), dùng chung hộp thoại/route với hai sàn kia.
- **Hạ tầng**: cron refresh token (`workers/token-refresh.ts`), dọn `tiktok_webhook_logs`, sức khỏe HQ + nhật ký webhook HQ có nguồn `tiktok`, kiểm toán phí rổ #3, tra mã vạch tự chữa lành, quét đơn hoàn kho. Log `[TikTok] Hình dạng …` in tên trường payload thật lần đầu mỗi lượt (tắt `TIKTOK_SHAPE_LOG=0`).
- **Chưa làm** (cần scope/API khác): chat & đánh giá TikTok (CSKH), Ads TikTok, affiliate theo creator (scope Affiliate), hóa đơn theo yêu cầu người mua.
