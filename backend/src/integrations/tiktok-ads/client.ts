...
// ---------- Nhóm quyền CAMPAIGN (TikTok duyệt 19/09/2026 — khuôn đã xác minh bằng probe cùng ngày) ----------
// Token cấp TRƯỚC 19/09/2026 không có quyền này → sàn trả 40001 "advertiser does not grant you …" → chủ shop bấm Kết nối lại.

/** 40001 kèm câu "does not grant you" = token thiếu nhóm quyền của endpoint (khác 40001 do tham số sai). */
export function isTiktokAdsScopeMissing(err: unknown): boolean {
  return err instanceof TiktokAdsApiError && err.code === 40001 && /does not grant you/i.test(err.message);
}

/** Cấu hình một chiến dịch GMV Max (/campaign/gmv_max/info/) — chỉ khai các trường Hubsell dùng. */
export interface GmvMaxCampaignInfo {
  campaign_id: string;
  ...
  store_id?: string;
  /** ROI mục tiêu; vắng khi chiến dịch chạy phân phối tối đa. */
  roas_bid?: number;
  /** Ngân sách ngày (đồng). */
  budget?: number;
  /** Sản phẩm (SPU) của chiến dịch — nguồn chính xác cho "sản phẩm nào thuộc chiến dịch nào". */
  item_group_ids?: string[];
  /** ALL_PRODUCTS | CUSTOMIZED_PRODUCTS */
  product_specific_type?: string;
  /** AUTO_SELECTION = sàn tự chọn video (điều kiện để loại / khôi phục video qua API). */
...
export async function getGmvMaxCampaignInfo(
  ...
  advertiserId: string,
  campaignId: string
): Promise<GmvMaxCampaignInfo> {
  return adsGet<GmvMaxCampaignInfo>("/campaign/gmv_max/info/", accessToken, {
    advertiser_id: advertiserId,
    campaign_id: campaignId,
  });
}

export interface GmvMaxBidRecommendation {
  roasBid: number | null;
  budget: number | null;
}

/**
 * ROI mục tiêu + ngân sách ngày SÀN gợi ý cho một chiến dịch Product GMV Max.
 * ⚠️ Probe 19/09/2026 (gian nhà): con số KHÔNG theo sản phẩm — 3 SPU thật khác nhau lẫn một SPU không tồn tại đều ra 7,2 ·
 * 6.000.000; 2 SPU một lượt ra ROI 4; không gửi SPU ra 5. Chỉ dùng làm số THAM KHẢO PHỤ ("mức sàn gợi ý chung"), không dựng
 * cột theo từng sản phẩm, không lấy làm căn cứ ra quyết định — căn cứ là ROI hòa vốn tính từ đơn đã đối soát (breakeven.ts).
 */
export async function getGmvMaxBidRecommendation(
  accessToken: string,
  input: { advertiserId: string; storeId: string; itemGroupIds: string[] }
): Promise<GmvMaxBidRecommendation> {
  const data = await adsGet<{ roas_bid?: number; budget?: number }>("/gmv_max/bid/recommend/", accessToken, {
  ...
  return { roasBid: num(data.roas_bid), budget: num(data.budget) };
}
