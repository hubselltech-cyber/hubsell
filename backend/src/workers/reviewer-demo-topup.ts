...
//
// Bối cảnh: hồ sơ ISV Shopee (nộp 10/09/2026) rồi Xét duyệt ứng dụng TikTok
// (nộp 16/09/2026, 10-12 ngày làm việc) khai tài khoản trial reviewer@hubsell.vn.
// Tài khoản này do scripts/seed-isv-reviewer.ts sinh: 3 gian Shopee + TikTok +
// Lazada KHÔNG có token thật, 62 ngày đơn neo theo ngày chạy seed. Không ai bồi thì sau 2-3 ngày Tổng quan hiện "hôm nay 0 đơn", đơn
...
const SETTLE_DAYS: Record<ChannelName, number> = { SHOPEE: 1, TIKTOK: 1, LAZADA: 2, OFFLINE: 1 };
...
/**
 * Trọng số đơn theo sàn (như seed: Shopee 50 / TikTok 27 / Lazada 23). Gian
 * nào không có thì chia lại theo các gian còn lại (chỉ Shopee+Lazada → Shopee
 * ≈ 68%). Lệch quá ±0,2 so với trọng số chuẩn thì cân lại.
 */
const CHANNEL_WEIGHT: Record<ChannelName, number> = { SHOPEE: 0.5, TIKTOK: 0.27, LAZADA: 0.23, OFFLINE: 0 };
const SHARE_TOLERANCE = 0.2;
...
const CARRIERS: Record<ChannelName, { carrier: Carrier; name: string }[]> = {
  SHOPEE: SHOPEE_CARRIERS, TIKTOK: TIKTOK_CARRIERS, LAZADA: LAZADA_CARRIERS, OFFLINE: LAZADA_CARRIERS,
};
/** Tiền tố mã vận đơn chiều đi / chiều hoàn theo sàn (giống mã thật). */
const TRACKING_PREFIX: Record<ChannelName, string> = { SHOPEE: "SPXVN0", TIKTOK: "8619", LAZADA: "LEXVN0", OFFLINE: "HS" };
const RETURN_PREFIX: Record<ChannelName, string> = { SHOPEE: "SPXVN", TIKTOK: "JTR", LAZADA: "LEXVN", OFFLINE: "HS" };

/** Chọn sàn cho một đơn theo trọng số chuẩn, chia lại trên các gian đang có. */
export function pickChannel<T extends { channelName: ChannelName }>(key: string, channels: T[]): T {
  const total = channels.reduce((sum, c) => sum + CHANNEL_WEIGHT[c.channelName], 0) || 1;
  let r = hashUnit(key, "chan") * total;
  for (const c of channels) {
    r -= CHANNEL_WEIGHT[c.channelName];
    if (r < 0) return c;
  }
  return channels[channels.length - 1];
}
/** Tỷ lệ chuẩn của một sàn trong tập gian đang có. */
function targetShare(name: ChannelName, channels: { channelName: ChannelName }[]): number {
  const total = channels.reduce((sum, c) => sum + CHANNEL_WEIGHT[c.channelName], 0) || 1;
  return CHANNEL_WEIGHT[name] / total;
}
...
export function settlementFees(orderId: string, channel: ChannelName, actualRevenue: number) {
  ...
  const isTiktok = channel === ChannelName.TIKTOK;
  // TikTok VN: hoa hồng ~4%, phí giao dịch ~5%, phí dịch vụ SFP ~4% (cùng seed).
  const fixedFee = Math.round(actualRevenue * (isShopee ? 0.04 : isTiktok ? 0.04 : 0.03));
  const paymentFee = Math.round(actualRevenue * (isShopee ? 0.045 : isTiktok ? 0.05 : 0.0245));
  const serviceFee = Math.round(actualRevenue * (isShopee ? 0.06 : isTiktok ? 0.04 : 0.05));
...
export async function runReviewerDemoTopup(opts: { email: string; now?: Date }): Promise<TopupSummary | null> {
  ...
  // (TikTok không có ví sàn — "ví" = đợt chi tiền đang xử lý, seed đã ghi).
  for (const ch of channels) {
    ...
    await prisma.channel.update({
      ...
      data: {
        lastSyncAt: now,
        ...(ch.channelName === ChannelName.TIKTOK ? {} : { walletBalanceSyncedAt: now }),
        ...(payout !== 0 && ch.channelName !== ChannelName.TIKTOK ? { walletBalance: { increment: payout } } : {}),
...
async function ageOrders(
  ...
  const chanOf = (channelId: string): ChannelName => channelName.get(channelId) ?? ChannelName.SHOPEE;
  ...
  for (const o of pending) {
    ...
    await prisma.order.update({
      where: { id: o.id },
      data: {
        shippingStatus: ShippingStatus.PROCESSED, packedAt, trackingCode: trackingCodeFor(o.id, chanOf(o.channelId)),
        // Kiện TikTok (packages[0].id) cho nút In vận đơn.
        ...(chanOf(o.channelId) === ChannelName.TIKTOK ? { platformPackageId: `1160${fnv1a(o.id).toString().padStart(10, "0")}` } : {}),
      },
    });
    s.processed++;
  }
    ...
    await prisma.$transaction([
      prisma.order.update({
        ...
        data: {
          ...
          returnTrackingCode: `${RETURN_PREFIX[ch]}R${fnv1a(o.id).toString().slice(0, 9)}`,
...
function trackingCodeFor(orderId: string, channel: ChannelName): string {
  return `${TRACKING_PREFIX[channel]}${fnv1a(orderId).toString().padStart(10, "0")}`;
}
...
async function createOrdersForDay(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const channel = pickChannel(key, channels);
    ...
    const carrierPick = hashPick(key, "carrier", CARRIERS[channel.channelName]);

    await prisma.order.create({
      data: {
        channelId: channel.id,
        orderCode: orderCodeFor(key, createdAt, channel.channelName),
...
// ============================================================
const WORKER_CODE_RE = /^(\d{15}|\d{18}|\d{6}[0-9A-Z]{8})$/;
...
async function rebalanceChannels(channelIds: string[], channels: DemoChannel[], now: Date, s: TopupSummary) {
  ...
  const others = channels.filter((c) => c.channelName !== ChannelName.SHOPEE);
  if (!shopee || others.length === 0) return;
  const SHOPEE_SHARE = targetShare(ChannelName.SHOPEE, channels);
  const SHOPEE_SHARE_MIN = SHOPEE_SHARE - SHARE_TOLERANCE;
  const SHOPEE_SHARE_MAX = SHOPEE_SHARE + SHARE_TOLERANCE;
  ...
  for (const [, orders] of byDay) {
    ...
    const pool = orders
      .filter((o) => (toShopee ? o.channelId !== shopee.id : o.channelId === shopee.id))
      .sort((a, b) => hashUnit(a.id, "flip") - hashUnit(b.id, "flip"))
      .slice(0, Math.abs(wantShopee - shopeeCount));

    for (const o of pool) {
      // Về Shopee, hoặc từ Shopee sang một gian khác chọn theo trọng số.
      const target = toShopee ? shopee : pickChannel(o.id, others);
      const carrierPick = hashPick(o.id, "carrier", CARRIERS[target.channelName]);
      await prisma.order.update({
        where: { id: o.id },
        data: {
          channelId: target.id,
          orderCode: orderCodeFor(o.id, o.createdAt, target.channelName),
          carrier: carrierPick.carrier,
          shippingCarrierName: carrierPick.name,
          ...(o.trackingCode ? { trackingCode: trackingCodeFor(o.id, target.channelName) } : {}),
          ...(o.returnTrackingCode
            ? { returnTrackingCode: o.returnTrackingCode.replace(/^(SPXVN|LEXVN|JTR)/, RETURN_PREFIX[target.channelName]) }
            : {}),
...
/** Mã đơn giống sàn thật: Shopee yymmdd + 8 ký tự; TikTok 18 số bắt đầu 5860; Lazada dãy 15 số. */
function orderCodeFor(key: string, createdAt: Date, channel: ChannelName): string {
  const h1 = fnv1a(`${key}:a`), h2 = fnv1a(`${key}:b`);
  if (channel === ChannelName.TIKTOK) {
    return `5860${String(h1).padStart(10, "0").slice(0, 7)}${String(h2).padStart(10, "0").slice(0, 7)}`;
  }
  if (channel === ChannelName.SHOPEE) {
...
async function upsertDailyCosts(userId: string, channels: DemoChannel[], now: Date) {
  ...
  // Ads tiêu dần theo giờ trong ngày (upsert → mỗi nhịp số nhích lên). TikTok
  // chưa có Ads trong Hubsell → không ghi AdSpend.
  for (const ch of channels) {
    if (ch.channelName === ChannelName.TIKTOK) continue;
