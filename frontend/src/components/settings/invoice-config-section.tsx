...
/**
 * TRANG HÓA ĐƠN ĐIỆN TỬ & CHỮ KÝ SỐ (Multi-Vendor Adapter) — đóng gói độc lập.
 *
 * Bốn khối, chia 2 nhóm theo công tắc `isInvoiceModuleEnabled`:
 *   1. Báo cáo & Giám sát phôi   — LUÔN MỞ (đầu trang, cảnh báo sắp hết phôi).
 *   2. Cấu hình NCC & chữ ký số  — khóa khi công tắc TẮT.
 *   3. API Key theo gian hàng    — khóa khi công tắc TẮT.
 *   4. Mua gói phôi (thương mại) — LUÔN MỞ (cuối trang, nhận cuộn từ "Nạp thêm").
 *
 * Khóa bí mật chỉ hiển thị dạng che; để trống khi lưu = giữ nguyên khóa cũ.
 */
export function InvoiceConfigSection({
  ...
  // Công tắc tổng của module — mặc định TẮT: chỉ khóa 2 khối KỸ THUẬT (cấu hình
  // NCC + API Key); khu thương mại (Báo cáo phôi, Mua gói) luôn mở.
  ...
  return (
    <div className="space-y-6">
      {/* ===== KHỐI 1: BÁO CÁO & GIÁM SÁT PHÔI — LUÔN HIỂN THỊ Ở ĐẦU TRANG,
          không phụ thuộc công tắc module; nút "Nạp thêm" cuộn xuống khối mua. */}
      ...
      <div className="flex max-w-2xl items-center justify-between gap-4 rounded-lg border bg-card p-4 shadow-sm">
        <div>
          ...
          <p className={cn(TEXT_SUB, "mt-0.5")}>
            Bật để cấu hình nhà cung cấp, chữ ký số và API Key theo gian hàng.
          </p>
        </div>
        <Switch
      ...
      {/* Fieldset CHỈ khóa 2 khối KỸ THUẬT khi module tắt (disabled chặn bàn
          phím, pointer-events-none chặn chuột, opacity làm mờ có transition).
          Khu thương mại (Giám sát phôi + Mua gói phôi) nằm NGOÀI fieldset —
          luôn mở để chủ shop theo dõi tồn phôi và nạp tiền bất cứ lúc nào. */}
      <fieldset
      ...
      >
        {/* ===== KHỐI 2: NHÀ CUNG CẤP & CHỮ KÝ SỐ (cấp shop) ===== */}
          ...
          <CardContent className="space-y-5 pt-5">
            ...
            <div className="grid gap-2">
              ...
              <p className={TEXT_SUB}>
                Mã đại lý ISV của Hubsell — <b>cố định, không chỉnh sửa</b>. Nhờ
                mã này mà mọi hóa đơn phát hành qua hệ thống được nhà cung cấp
                ghi nhận thuộc đại lý Hubsell (hưởng hoa hồng ISV).
              </p>
            </div>
      ...
      {/* ===== KHỐI 4: MUA GÓI PHÔI HÓA ĐƠN (Ưu đãi đối tác Hubsell) —
          LUÔN MỞ để chủ shop nạp tiền bất cứ lúc nào. ===== */}
