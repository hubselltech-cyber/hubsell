...
export interface PnlWaterfallProps {
  ...
  net: number;
  /** Chiều cao vùng vẽ (px) — bỏ qua khi fill */
  height?: number;
  /**
   * Giãn theo chiều cao còn lại của card (cha là flex-col): đặt cạnh card cao
   * hơn (Tỷ trọng kênh bán) thì chart kéo dài theo thay vì hở đáy.
   */
  fill?: boolean;
  className?: string;
}
...
export function PnlWaterfall({
  ...
  return (
    <div
      className={cn("w-full", fill && "min-h-64 flex-1", className)}
...
export function PnlWaterfallFooter({
  net,
  gross,
}: {
  net: number;
